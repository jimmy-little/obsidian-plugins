...
/** Primary people directory (first configured path). */
export function primaryPeopleDir(peopleDirs: string[]): string {
	return peopleDirs.map((d) => d.trim()).filter(Boolean)[0] ?? "";
}
